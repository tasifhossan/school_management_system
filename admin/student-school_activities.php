...
$allowed_sort_columns = ['title', 'activity_date', 'created_by_name'];
...
/**
 * Retrieves all school activities with filtering and sorting.
...
function get_all_activities($conn, $search = '', $sort_col = 'activity_date', $sort_dir = 'DESC') {
    ...
    // Base SQL query
    $sql = "SELECT 
                sa.id, sa.title, sa.activity_date, u.name AS created_by_name
...
<main class="content">
    <div class="container-fluid p-0">
        <h1 class="h3 mb-3">School Events & Activities</h1>
                    ...
                    <div class="card-header">
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="GET" class="mt-2">
                             <div class="input-group">
                                <input type="text" name="search" class="form-control" placeholder="Search by Activity Title, Description, or Organizer..." value="<?php echo htmlspecialchars($search_query); ?>">
                                <button class="btn btn-primary" type="submit">Search</button>
                                <?php if (!empty($search_query)): ?>
                                    <a href="student-school_activities.php" class="btn btn-secondary">Clear</a>
                                <?php endif; ?>
                            </div>
                        </form>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover my-0">
                                <thead>
                                    <tr>
                                        <th><?php echo sortable_header('Activity Title', 'title', $sort_column, $sort_direction, $search_query); ?></th>
                                        <th><?php echo sortable_header('Date of Activity', 'activity_date', $sort_column, $sort_direction, $search_query); ?></th>
                                    ...
                                    <?php if (empty($activities_data)): ?>
                                        <tr>
                                            <td colspan="4" class="text-center p-4">
                                                <?php echo !empty($search_query) ? 'No activities found matching your search.' : 'There are no upcoming activities listed.'; ?>
                                            </td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($activities_data as $row): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="student-view-activity.php?id=<?php echo $row["id"]; ?>" class="btn btn-sm btn-outline-primary">View Details</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
